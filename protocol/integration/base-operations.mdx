...
<Note>
  **Common failure.** Calling `redeemById` before `unlockTime` reverts with `ERC4626ExceededMaxRedeem` (the hub's `maxRedeem(controller)` returns 0 during bonding). See [Gotchas](/protocol/integration/gotchas#erc4626exceededmaxredeem-on-premature-claim-uty-only).
</Note>
